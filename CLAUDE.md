...
- ⭐**【2026-08-25 mtさん決定】目標は「1年で100テナント」＝ペース換算で週2。** ただし**数字で固定しない**――「週どれくらい実際にできるか」と「AdSenseに問題が出ないか」を見て決める。⭐**【2026-09-05 mtさん確定】週2・1年で100テナント・その後は放置運転を目指す、で決定。** 下の行の旧上限（月3〜4本＝週1・年50本）と競合していたが、**この行を正**とし、旧上限は「承認前〜直後の安全値」として読む。⚠️ただし①〜④の運用（週1から始めてGSCを見ながら週2へ／AI埋めテナントを連続させない／異常が出たら即落とす）は変えない——mtさんの意図は「早く放置状態に持っていく」ことであって「無理に週2を維持する」ことではない。
...
- ⚠️**【2026-09-05 訂正】新テナントの初期10問の作り方は「AIに10問答えさせる」ではない。** ⭐正しい手順は Notion「テナント作成マニュアル」6章の**分業**：①**候補集め＝プランナーのClaudeが20〜30問**（そのジャンルで実際に検索・質問されている内容を調べる。⛔文章はコピーせず傾向を拾って書き起こす）→ ②**選別＝エンジニアのClaudeが `calibrate-threshold.mts` に通し、閾値91超えかつ回答が正しい問だけに絞る** → ③**選定と清書＝mtさんが10問を選び自分の言葉に直す** → ④**投稿＝mtさんが実サイトのフォームから**（⛔SQL一括投入はしない。タイムスタンプが偽物になりAI回答・8言語翻訳・マッチングを全部バイパスする）→ ⑤数日に分散（レート制限）。⛔**③をClaudeに代筆させないこと**——Googleが規制するのは「人間のレビューを経ていない大量生成コンテンツ」で、③の本人による選定・清書＝人間のレビューと価値付加が**ポリシー違反にならない根拠そのもの**。⚠️テナント量産時に一番省略したくなる工程。⛔また「回答なしで10問置く」もダメ：②の選別を飛ばすと開設時に無回答だらけの死んだテナントに見える（新テナントは回答者候補がいないので、スコア91未満は人間ルート→候補ゼロ→自動で🔥高難度＝回答ゼロのページになる）
- **将来スケール（〜100テナント構想）**：承認後は"宣伝1発→放置"でスロー展開。開設ライン10問は上の分業手順で満たす（人間回答は後から実ユーザーが埋める）。**1〜3問しかないテナントはnoindexのまま**（薄いページはドメイン全体のSEO/審査を下げる）。展開ペースは**承認まで&直後は月1〜2本**（⚠️旧記述「その後は月3〜4本＝週1・年50本まで」は 2026-08-25 の決定で上書き済み。⭐承認後は週1から始めてGSCを見ながら週2へ＝667行が正）。一気の量産はscaled-content判定リスク。判断軸はカレンダーでなく質と各テナントの生存シグナル。Search Consoleで手動対策/インデックス異常を監視（カナリア）。
...
- ⚠️⚠️**【2026-08-25 実データで反例が出た】「ブレはAI公開率が下がる方向にしか効かない＝安全側」は誤りだった。** Day 9 で**2問が同時に逆方向へブレた**：
  ...
  - **実際の配分の変化（予測）**：🤖22→22問／📝8→7問／🔥6→7問＝回答ゼロ9%→10.4% ⚠️**【2026-09-05 訂正】この予測は外れた。** G22は🔥に流さず📝として回答したので、⭐**最終実績は 回答ゼロ 6問(9%)のまま**。最終値は solved 34／📝6／🤖21／🔥6 ＝67問
...
**⏭️ 承認後にやること** ⚠️**【2026-09-05 訂正】見出しにあった「投稿期間中はpushしないので」は誤り。** ⛔そんな方針は存在しない（47行の標準許可のとおりUI微修正はpushしてよい）。⭐正しい理由は「E2＝申請後は審査中なのでpushを止める」。なお投稿は2026-09-05に完了済み
...
⭐**結論：申請前に触るのは E0 だけでよい**（＋ついでの E0b）。他は承認後。 ✅**2026-09-05 に E0・E0b とも完了・本番反映済み。**
...
1. ⛔**失敗した下書きが、そのブラウザに残り続ける（＝今回mtさんが踏んだのはこれ）。**
   localStorageは**ブラウザごと**なので、奥様のブラウザで429になった下書きは、
   **本人のブラウザで同じ問を投稿し直しても消えない**（2アカウントをSafari/Chromeで使い分けているため）。
   ⚠️mtさん報告：「マッチングしている妻の方に出ます」＝奥様側のブラウザに残っていた
   - **当面の対処**：奥様のブラウザで「質問する」を開き、**タイトルと本文を両方とも空にする**
2. ⛔**下書きがテナント単位で、アカウント単位ではない（潜在）。** `draftKey` に user_id が無いので、
   **同じブラウザで**奥様と本人を切り替えれば下書きが混ざる。⚠️今は別ブラウザなので顕在化していないが、
   実ユーザー段階で「家族で1台」なら起きる
   - 修正案：`wa_draft_${tenantId}_${userId}`。⚠️未ログイン時のキーをどうするかは要検討
3. ⛔**「下書きを消す」導線が無い。** 消すには**タイトルと本文を両方とも空にする**しかない
...
- ⚠️**当面の運用**：投稿に失敗した（429など）ときは、そのブラウザでタイトルと本文を両方空にしてから閉じる

---

### ⛔ 2026-08-30 投稿中のインジケーターが無い ―― ⚠️**2026-09-05に撤回（実装済みだった）**

⭐9/5未明の投稿でスクショに「AIが考え中...」「送信中...」のオーバーレイが確認できた。⛔プランナーが `setOverlay(...)` の呼び出し位置だけ見て「APIが返った後にしか出ない」と誤読していた。**タスクではない。** 以下は誤った記録として残す：

**mtさん**：「投稿長かったので、AI回答中みたいなアニメで、送信中を出しましょうか？」

- ⚠️実態：投稿ボタンを押してから応答まで**10〜20秒かかる**（AI判定＋7言語翻訳を直列でawaitするため）。その間の画面表示が無く、**押せたのか止まったのか分からない**
- `QuestionForm.tsx` の `setOverlay(...)` は**APIが返ってきた後**に出るので、待っている間には効いていない
- ⭐修正案：送信開始と同時にオーバーレイを出す（「AIが確認しています…」等）。⚠️2パス化（AI判定の検証パス）を入れるとさらに +3秒 伸びるので、**セットで必要になる**
- ⛔承認前には触らない。⭐実ユーザーが来る前には必須（20秒の無反応は離脱に直結する）

---

### 🔍 2026-08-31 Day 15 全件監査 ── 54/67問

```
✅ 質問54問・回答50件（計158項目）すべて 7/7・欠落ゼロ
✅ 質問と回答の紐づけ 全件正しい
集計  solved 23／ai_answered 17／open 10／hard 4
回答ゼロ 4問 = 🔥高難度枠（M05・G16・G14・M29）＝すべて意図どおり
```

- ⚠️**G28「マーシャルのdsl1の音量について。」が 較正95→本番75 で👤に落ちた。** ⭐原因は `gemini.ts:423` のヘッジ語ペナルティと推定（**95 − 20 = 75** でぴったり合う）。「マンションで鳴らせるか」は環境依存なのでAIが曖昧に書き、減点されて91を切った。⛔**引き直しても同じ理由で落ちる**ので、mtさんが回答して解決
- ⭐**🤖予定が👤に落ちた率＝19問中5問（26%）**（M13 95／G22 90／M25 90／M27 90／G28 75）。対応は「落ちたらmtさんが答える」で固定。根本対策は承認後のAI判定ロジック改良

#### ℹ️ 誤解しやすい仕様（2026-08-31に実際に迷った）

```
「回答期限が過ぎたため、この質問への回答受付は終了しました」
   = 新しい回答を受け付けないという意味だけ
```
- ⭐**BAは押せる**（`page.tsx:385` の条件は `isOwner && !isSolved && !a.is_accepted` のみで、期限は入っていない）
- ⭐**期限切れでも回答が1件でもあれば `hard` には上がらない**（`auto_escalate_expired()` は「担当者が期限切れまで未回答」の質問だけを対象にする）
- ⚠️M06「Genelec G Three…デスクトップモード」で「高難度になったのでは」と心配したが、実際は `open` のままBAを押せた

---

### ⭐⭐ 2026-09-05 投稿完了 67/67問 ── と、その日に見つかった重大バグ

**9/3・9/4ぶんを9/5未明にまとめて6問投稿し、67問すべて完了。**

```
✅ 投稿  67/67問
✅ 翻訳  質問67問（×2）＋回答61件 = 195項目 すべて7/7・欠落ゼロ
✅ 回答ゼロ 6問 = すべて🔥高難度枠（M05・G16・G14・M29・M32・G17）＝意図どおり
⏳ BA 7件が残り（9/5の夜にまとめて押す＝mtさん決定）
```

#### 🚨 この日に見つかった2つのバグ（両方その場で修正）

**① Gemini 503（Google側の障害）**
```
"This model is currently experiencing high demand. Spikes in demand are usually temporary."
```
⚠️6問を7分に固めた（RPM 15 超過）ことが引き金だが、**503自体はGoogle側**。翻訳もAI判定も落ち、
**質問6問・回答5件の翻訳が0/7**、**score=0 が4件**（M28・M30・M33・M08）発生。
⭐M08は🤖予定だったが score0 で👤に落ち、Notionにあったmtさんの回答を貼って解決。

**② ⛔Groqのフォールバックがモデル廃止で死んでいた（E0の真因）**
```
groq 404 "The model `llama-3.1-8b-instant` does not exist or you do not have access to it"
```
- ⭐**Groqが llama 系を全廃**していた。gemini.ts の `llama-3.3-70b-versatile` も同様に404
- ⚠️**Groqは「Geminiが落ちたときだけ」呼ばれる**ので、死んでいても普段は気づけない。
  ①が起きて初めて表に出た＝**保険が必要な日に保険が無かった**
- ⛔2026-08-26に「Groqへのフォールバックが走っていない可能性」と書いたが、**理由の推定が外れていた**。
  「予算切れで呼ばれない」ではなく「呼ばれても404」だった（両方あった）

#### ✅ 修正内容（ローカルで動作確認済み・⚠️本番へのpushは未実施）

```
① src/lib/translate.ts:30
...
```
2026-09-05時点で実在：`openai/gpt-oss-120b` / `gpt-oss-20b` / `qwen/qwen3.6-27b` / `qwen/qwen3.8-27b` / `groq/compound` ほか
...
- ⚠️**枠は3モデルとも同じ**：リクエスト 1,000/日・トークン **8,000 TPM**。⭐差がつくのはTPMで、qwenだけ1回の出力で詰まる
- ⭐1問の翻訳＝7言語で約5,300字。実運用1日6問なら桁違いに余裕がある

#### ⚠️プランナーの反省

- ⛔「朝まで待てば直る」と言ったが**待っても直らない別のバグ**だった。⭐エラーの中身を見る前に原因を推測した
- ⛔**質問側のバックフィルが無いこと**に、6問が0/7になるまで気づいていなかった
- ⚠️「Groqが半年死んでいた」と書いたが、**プロジェクトは6/17開始で約2ヶ月半**。数字を確認せずに書いた

---

### ⭐ 2026-09-02 1102の根本原因を特定（「ビルドのクジ」とは別の第2の原因）

**決め手は「投稿直後だけに出る」こと。**
```
23:15  測定 → 16回中0エラー（正常）
23:19-21  3問投稿（AI判定＋AI回答＋7言語翻訳）
23:21  1102発生 → 測ると 24回中1回（4%）
```
⚠️前日8/31の15%とは**重さもタイミングも違う**。あれはビルドのクジ、これは別物。

#### 原因（`src/app/api/questions/route.ts` で確認）

```ts
129行  await askWithScoreInScope()   AI判定
240行  await askWithScore()          AI回答生成
253行  await translateToLocales()    7言語翻訳
322行  return NextResponse.json()    ← ここでやっと返す
```
⛔**全部を直列awaitしてから返している**ので、**1投稿がWorkerを10〜20秒占有する**。
その間に来たアクセスがCPU上限に当たって1102になる。

#### ⭐根本解決策（承認後に実装）

| | 案 | 評価 |
|---|---|---|
| ⭐① | **翻訳を投稿から切り離して後追いにする**（投稿は3秒で返し、翻訳はcronで拾う） | ⭐**採用候補。** 既に `scripts/backfill-answer-translations.mts` があるので仕組みは揃っている。⚠️投稿直後の数分だけ多言語ページが原文のまま。⭐副次効果で投稿が20秒→3秒＝「投稿中インジケーターが無い」問題もほぼ解消 |
| ② | Cloudflare Queues に逃がす | 正攻法だが要調査（無料枠で使えるか未確認） |
| ③ | AI回答も非同期化 | ⛔「AIが答えた」の即時性が失われるので不採用 |

- ✅**AdSenseへの影響はない。** この1102は「投稿するから」出るもので、⭐**申請時は投稿しないので出ない**
- ⚠️やることは「**投稿完了後（9/4以降）に、投稿していない状態で30〜50回測って0%を確認**」だけ。⭐申請前チェックリストに入れる
- ⭐**1102には2つの原因がある**と整理しておくこと：**(a) ビルドのクジ**（対処＝再ビルド）と **(b) 投稿の重い処理**（対処＝案①）。⛔混同すると「再ビルドしたのに直らない」と誤診する

---

### ✅ 2026-09-02 クロールされない件 ── 設定は正常だった

mtさん「全然クロールされないが、クロールされないようになってたりしないか」を受けて実機確認。

```
✅ robots meta なし ＝ index可
✅ sitemap に当日の新問も収録済み（root 10／dtm 74／guitar 72 URL）
✅ HTTP 200
⛔ ブロックしている設定は無い
```
⚠️GSCの数字が動かないのは**レポートの更新が数日遅れる**ため（8/27に「最終更新日が6日古い」ことを確認済み）。
⭐**GSCを見るときは必ず最終更新日を先に確認する。**

---

### 🚨 2026-08-31 1102が再発 → 再ビルドで解消（投稿フェーズ中の初発生）

```
22:40  guitar.wisdomassemble.com で Error 1102（Ray a33c71fb2ebad779）
22:41  単発20回中3回エラー（15%）＝⛔恒久ルールの異常判定（0%台が正常・20%超が異常）
22:43  同じソースで再ビルド（空コミット 8c0f39f を push）
22:48  単発45回中0回エラー（0.0%）＝✅解消
```

- ⚠️**直前のpushは c11d83a（docsのみ）。** ⛔**docsだけのpushでもWorker全体が再ビルドされる**ので、どのpushでも起こり得る。7/28の前例（ソース無変更でビルドだけ壊れて1102が6割）と同じ型
- ⭐**対処は再ビルド一択。** 記録どおり空コミットのpushだけで直った

#### ⛔ 課金（Workers Paid $5）は解決策ではない ―― プランナーが一度提案して撤回した

2026-08-31にプランナーが「確実に潰すならWorkers Paid」と提案したが、**mtさんの指摘どおり誤り**。
`2026-07-30【解決・調査終了】`に **「Workers Paid $5は不要と確定」** と既に書いてある：
```
平常時のトラフィックは 0.024 req/s
実測で少なくとも 26 req/s までエラーなし ＝ 約1,000倍の余裕
```
⛔**1102の原因はCPU上限ではなく、7/30に潰し済み**（アイコンのプローブ／Link先読み／OGP生成の静的化で
Median CPU 141.12ms→44.12ms）。負荷テストも実施済みで、容量の問題ではない。
⭐**残っているのは「ビルドのクジ」だけで、それは課金では直らない。**

- ⚠️**プランナーの反省**：記録に結論があるのに読まずに提案した。⭐**1102の話が出たら、まず `2026-07-30【解決・調査終了】` を読む。**
- ✅**申請前の運用は変えない**：pushしたら単発5〜10回＋CloudflareのError Rate確認。異常なら再ビルド、直らなければRollback

#### ⭐【2026-08-31 mtさん決定】1102への対処方針

> 「アドセンス審査中や運用がある程度始まったら、pushはメンテナンスの時間以外は厳禁にすれば再現しない。そうなると案Aが一番有効」

因果はこう：**push → Workerが再ビルド → ⚠️確率的に壊れる**。裏を返すと **pushしなければ新しいビルドは作られない＝壊れようがない**。

| | 方針 | 時期 |
|---|---|---|
| ⭐1 | **push はメンテナンス時間のみ**（承認後の運用ルールとして厳格化） | 承認後 |
| ⭐2 | **案A：push後の自動チェック＋異常なら自動で再ビルド** — 人が気づく前に直す | 承認後に実装 |
| ⭐3 | **プランナーのpushを1日1回にまとめる** | ⭐今すぐ |

- ⚠️**3が今いちばん効く。** 2026-08-31にプランナーは**1日で5回push**した（すべてCLAUDE.mdの記録）。⛔記録のたびにビルドのクジを引いていた。**1日の終わりにまとめて1回**にする
- ⚠️投稿フェーズ中に本番へpushしているのは実質プランナーだけ（アプリのソースは8/24以降ノータッチ）。⭐**つまり1102のきっかけを作っているのはプランナーのpush**
- ⛔根本原因（なぜビルドが壊れるか）は未特定のまま。Cloudflare側のビルドが非決定的で、こちらからは見えない。⭐**原因を消すのではなく「壊れたビルドを本番に居座らせない」方向で解く**

---

### 🔍 2026-08-30 Day 14 全件監査

```
✅ 質問51問・回答47件  翻訳すべて 7/7（欠落ゼロ）
✅ 質問と回答の紐づけ  全件正しい
集計  ai_answered 17／solved 19／open 12／hard 3
回答ゼロ 4問 = 🔥3問（M05・G16・G14）＋ M29（🔥待ち）＝すべて意図どおり
```

**⚠️この日のスコアのブレ2件**
- G31「quad cortexで、アンプのIRの作り方を教えて」 予定✅BA → **95で🤖化**。⭐AI回答は正確（Neural CaptureとIRの違い／SM57のコーン位置／パワー管ドライブ／Capture Out→In）。⚠️ただし**mtさんの回答「マイクを使うな、ロードボックスで取り込め」という逆張りの一次情報が使えなくなった**のは惜しい。⛔引き直しはしない（枠を食う）
- M27「メロディを生み出すコツは？」 予定🤖 → **2回連続90で👤**。⭐mtさんが回答を書いて解決（散歩しながら鼻歌→スマホで録音、という実体験。AIには書けない内容）

**⚠️投稿漏れが1件あった**：G26「fenderのdeluxe reverbのtone masterと普通のdeluxe reverbの違いは？」が7問中1問だけ抜けていた。同日中に本人アカウントから投稿して回収。⭐**7問の日は数え漏れが起きる。投稿後に必ずDBの件数を数えること。**

**📌既知の食い違い（対応不要）**：M13「初心者にもわかりやすいDAWソフトってなに？」が🤖なのに`solved`。⚠️2026-08-19に予定どおりBAを押したもので、「🤖にBAを押さない」という方針はその後に決まった。1問だけなので放置。
...
### 🔍 2026-08-30 全件監査（44問・41回答）── ⭐中身の問題はゼロ

mtさん「質問と回答が間違っているとかはないんですよね？翻訳も投稿したものは平気ですね？」を受けて、
**投稿済み全件をDBから引いて突き合わせた。**

```
✅ 翻訳    質問44問のタイトル・本文、回答41件  すべて 7/7（欠落ゼロ）
✅ 紐づけ  質問と回答の対応  全件正しい（貼り間違いゼロ）
```
⚠️2026-08-19にM04で回答の貼り間違いを起こしているので、そこも含めて全件見た。

**回答ゼロは3問だけで、すべて意図した🔥高難度枠**：
```
M05「USBケーブルの質で音質は変わる？」
G16「organic soundsのthe towerとbig towerの違いは？」
G14「2025年のjapan限定のfender starmaster、音ってどんな感じ？」
```
...
────────────────
回答が読めるページ  41/44 = 93%
回答ゼロ           3/44 = 6.8%   ⭐AdSenseの危険水域は2割
```
⭐**スコアのブレは6件起きているが、入れ替わるだけなので相殺している。** 当初の目標配分
（🤖22／✅31／📝8／🔥6）から実質ずれていない。⛔慌てて調整する必要はない。

#### バグの棚卸し（AdSenseへの影響で仕分け）

| 見つかったもの | AdSenseへの影響 | 扱い |
|---|---|---|
| 下書きが残る（8/30） | ⭐**影響なし。そもそもバグではない**（429で失敗したので下書きが守られた正常動作） | 承認後に「破棄」ボタン |
| 削除しても投稿枠が戻らない | ⭐**影響なし**（運用の話・ページの中身に無関係） | 運用ルールで回避 |
| レート制限の窓が24時間 | ⭐**影響なし**（同上） | 夕方から投稿する |
| AI判定ロジック（固有名詞の捏造） | ⚠️**唯一の「中身」の問題** | 承認後の最優先（下記） |

⭐**結論：ページの中身が壊れているものは1つも無い。** 申請の障害になる要素は現時点でゼロ。
...
```
- ⛔⛔**削除しても投稿回数は戻らない（2026-08-30 実コードで確認）。** `/api/admin/questions/[id]` に `rate_limits` を戻す処理は**無い**（grepして0件）。つまり **投稿→削除→再投稿 = 枠を2つ消費する**
  - ⚠️**これは「スコアがブレたら削除して引き直す」戦略と正面からぶつかる。** M03・M25・M27でやったが、そのたびに枠を1つ余分に食っていた
  - ⭐**引き直しは「その日の投稿枠に余裕があるときだけ」。** 6問の日（土日）は最初から余裕が無いので、ブレても引き直さず**そのまま受け入れる**か翌日に回す
  - 📖実例：2026-08-30未明、M27「メロディを生み出すコツは？」を削除して再投稿しようとしたら**両アカウントとも上限**で投稿できず、翌日送りになった
- ⭐**回避策：もう一方のアカウントで投稿する。** 本人が投稿→奥様にマッチ→奥様が回答、で詰まない（⛔「奥様は答えられない」は誤り）
- ⭐**投稿は夕方から始める。** 深夜0時台に固めると窓がじりじり後ろへずれ、翌日その時刻まで空かない。⚠️実際に2026-08-29→30で詰まった
...
> ⭐**2026-09-05：ここから申請までの日割りは Notion「🎯 AdSense申請までのロードマップ（9/5〜9/22）—— mtさん／Claude 役割分担」にまとめた。**
> このチェックリストが「何をやるか」の正本、Notionのロードマップが「いつ・誰がやるか」の正本。
> ⚠️矛盾したらこちらのチェックリストが優先（Notionは見やすさのための写し）。

### 🏁 2026-09-05 投稿フェーズ完全終了（BA 7件 → 67問クローズ）

⭐**8/17〜9/5の19日間で67/67問。最終監査の実測値：**

```
solved(BA済) 34 ／ 📝回答のみ 6 ／ 🤖AI公開 21 ／ 🔥高難度 6 ＝ 67
未BAの残り 0件 ／ 翻訳195項目の欠落 0 ／ statusの整合異常 なし
回答が読めるページ 61/67 (91%) ／ 回答ゼロ 6/67 (9%)
```

⭐**Notionの「📋投稿ステータス一覧」の予定と完全に一致した**（✅BA選択31 = 達成29 + 🤖化した2／🤖22 = BA済1 + AIのみ21／📝は🤖化3問を引いて6／🔥6）。予定からのズレはすべて記録済みの理由（スコアのブレ）で説明がつく。

⚠️**この日にプランナーがやった間違い2件（同じ型なので繰り返さないこと）**
- ⛔**BA対象を13件と誤って提示した。** DBから「人間回答があってBA未選択」を機械的に抽出しただけで、**Notionの📝方針（回答するがBAは選ばない）と照らさなかった**。正しくは7件。⭐**DBの状態だけで判断せず、必ずNotionの割り当てと突き合わせる**
- ⛔**M33の「エレメンツ」を誤りとして修正を勧めた。** Elements のカタカナは「エレメンツ」で**正しい**（元の誤りは「エレメント」＝単数形で、mtさんが既に直していた）。⭐正規表現を `エレメンツ|エレメント` と両方に効かせたため、正しい方まで警告になった。**チェックを書くときは「直った状態」が引っかからないか確かめる**

⭐**この日の発見（実害なし・記録のみ）**：M13「初心者にもわかりやすいDAWソフトってなに？」だけ**AI回答にBAが押されている**（8/19・Day 3）。🤖22問中1問だけなので不自然ではなく、直さない。

#### 📅 2026-09-05 時点の残り（mtさん向けサマリ）

| いつ | mtさん | Claude |
|---|---|---|
| ~~今夜 9/5~~ | ✅**BA 7件完了＝67問クローズ** | ✅一覧提示／全件監査／Notion・MD記録 |
| 9/6〜9/12 | 🛑**マーク判断(W/WA/新規)**・note/dev.to/Pinterest開設・マスター1枚 | 登録手順（罠つき）／7サイズ切り出し／プロフィール文 |
| 9/13〜9/18 | 動画1本・note記事・dev.to記事（⛔本文はmtさんが書く） | 骨子と実数字／字幕／キャプション |
| 9/15前後 | GSC→ページ のスクショを送る（⭐まず最終更新日を見る） | スクショ判読／打ち手 |
| 9/20〜9/21 | URL 12件を個別リクエスト | 投稿ゼロで30〜50回測定／全67問監査／URL一覧 |
| **9/22** | **AdSense申請** 🎯 | 最終確認／⛔以後 push 停止（E2） |

⭐**mtさんの手が要るのは実質「BA」「マーク判断」「SNS/記事/動画」「GSCを1画面見る」「URL 12件」だけ。**
🛑**最大のボトルネックはマーク判断**（決まらないとアイコン7媒体・バナー5媒体が1枚も作れない）。

⛔**申請前にはやらないと決めたもの**（迷い直さないための記録）：B3 OAuth本番公開／B7 不適切コンテンツ対策／OG画像の英文差し替え（⭐審査でOG画像は見られない。⚠️ただしSNS宣伝の前には必須）／6言語のnoindex解禁／アフィリリンク／1102対策のコード改修／AI判定ロジックの改良。

✅**GSCの設定は問題なし（2026-09-05 実機で確認）**：ルート10URL・music 84URL・guitar 74URL・bug 0URL（休眠＝正しい）、robots.txt に全面ブロック無し、質問ページに robots meta 無し（＝index可）、canonical は自ページ、HTTP 200。⭐**クロールを妨げる設定は1つも無い。** GSCの数字が動かないのはレポートが数日遅れているだけ。
...
- [x] ~~**A5. 67問の投稿完了**~~ ✅**2026-09-05 完了（67/67問）。** 翻訳195項目すべて7/7。⏳残るBAは7件で9/5夜に押す。以下は当初の計画：（音楽36＋ギター31・約3週間）— mtさん・妻。配分は🤖22／✅BA選択31／📝回答のみ8／🔥高難度6＝**回答ゼロは6問(9%)に抑える**
...
**【E. タイミング】**
- [x] ~~**E0b. hreflangが8言語のまま絞られていない**~~ ✅**2026-09-05 修正済み**（`availableLocales` の絞り込みを `routing.locales`(8言語) → `INDEXABLE_LOCALES`(en/ja) に変更。型チェック通過）。⭐将来6言語を解禁するときは INDEXABLE_LOCALES を足すだけで sitemap・robots・hreflang が揃って追従する。以下は発見時の記録：（2026-08-27 実機で発見）。`sitemap` と `robots meta` は `INDEXABLE_LOCALES`（en/ja）で絞ったのに、**hreflangだけ8言語ぶん出ている**（`src/app/[locale]/questions/[slug]/page.tsx` の `availableLocales`）。コード内コメント「（sitemapと同基準）」も**もう同基準ではない**ので併せて直す。⚠️**実害はほぼゼロ**（Googleはnoindexを指すhreflangを無視するだけでペナルティは無い）。⛔単独でpushする価値は無い＝E0のついでに。
- [x] ~~⭐⭐**E0. 翻訳フォールバックの修正**~~ ✅**2026-09-05 完了。** 真因は推定と違い「⛔Groqが llama 系を全廃してモデルが404」だった。①モデルを Groq × openai/gpt-oss-120b へ差し替え ②Geminiのタイムアウトを予算の2/3(12秒)で切りGroqに1/3(6秒)残す ③質問側バックフィルを新設。⭐Geminiのキーを空にした強制テストで **7/7言語・2.4秒** で成功を確認。以下は発見時の記録：⛔**この窓を逃すと承認までpushできなくなる**（E2で申請後のpushを禁止しているため）。窓は**9/6の投稿完了〜9/22の申請開始まで**の2週間強。
...
- [ ] ⭐**【承認後】1102対策その2：翻訳を投稿APIから切り離す（案①）** — 投稿が10〜20秒Workerを占有しているのが第2の原因（2026-09-02特定）。投稿は3秒で返し翻訳はcronで拾う。⭐副次効果で「投稿中インジケーター」問題も解消。詳細は上の「⭐ 2026-09-02 1102の根本原因を特定」
- [ ] ⭐**【承認後】1102対策：push後の自動チェック＋自動再ビルド（案A）** — pushのたびにビルドが確率的に壊れる（7/28・8/31に発生、どちらも再ビルドで解消）。人が気づく前に直す仕組みを入れる。あわせて**承認後はpushをメンテナンス時間のみに限定**する運用ルールを敷く。⛔課金（Workers Paid）では直らない
- [x] ~~**【承認後・実ユーザー前】投稿中のインジケーターを出す**~~ ⛔**2026-09-05 撤回：そもそも実装済みだった。** 9/5未明の投稿でmtさんのスクショに「AIが考え中...」「送信中...」のオーバーレイが出ていることを確認。⚠️プランナーが `setOverlay(...)` の呼び出し位置だけを見て「APIが返った後にしか出ない」と誤読していた。⭐タスクではない
...
- [ ] ⭐**【承認後・AI判定改良と同じpushで】`src/lib/gemini.ts:26` の `GROQ_MODEL = 'llama-3.3-70b-versatile'` を生きているモデルに差し替える** — ⚠️E0で直したのは `translate.ts` の翻訳側モデルだけ。⛔こちらは**AI回答生成・スコア判定側のフォールバック**で、同じ理由（Groqがllama系を全廃）で404のはず（未確認）。Geminiが生きている間は表面化しないが、**Geminiが落ちたときにAI回答が全滅する**。⭐差し替え先は翻訳側と同じ `openai/gpt-oss-120b`（実測で採用済み・1,000リクエスト/日・8,000 TPMの枠は3モデル共通）
- [ ] **M33「iZotope Ozoneのバージョンの違いを教えて」の表記確認** — ⚠️「エレメント」→ **Elements**。★アフィリ枠なので製品名の表記は重要。⭐A4の誤字4件と同種だが、A4のリストには入っていなかった
