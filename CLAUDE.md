...
### 🚩 次セッションの入口（2026-07-27 時点・ここから読む）

**いまどこ**: 質問67問がFIXし、全問Gemini診断も完了。**ユーザーは👤45問の回答を執筆中**（Notionの各質問の下に直接書いている）。次にClaudeが動くのは、その執筆が終わってから。
...
**回答執筆が終わったら、Claudeがやること（この順）**:
1. **人間回答を入れた質問の再診断**（`npx tsx scripts/calibrate-threshold.mts scripts/calibration.{music,guitar}.json`。環境変数は不要）
2. **ギターテナント作成**（テナントビルダーで設定→コード反映→GENRE_CONFIG追加。閾値はDEFAULT_THRESHOLD=91が自動適用）
3. **BUGをルートから外して休眠化**（`PortalHome.tsx`の`REVIEW_TENANT_IDS`から`bug`を削除）
4. **投稿スケジュール＋貼るだけパケット作成**（何日にどの質問をどのアカウントで投稿/回答/ベストアンサー選択するか）
5. その後 AdSense申請前のトラフィック作り4つ（上記「⭐AdSense申請「前」のトラフィック作り」参照）
...
### 🤝 役割分担（2026-07-25 ユーザー確定・重要）
**このセッション（Claude Code）は「技術的・エンジニア的な実装のみ」を担当する。企画・コンテンツ作成はプランナー（別のClaude.aiセッション）と本人が行う。**
- **プランナーの担当**：質問の企画・収集・65問の処理・投稿タイムスケジュールの作成・宣伝の企画
- **本人の担当**：質問と回答の作成/検閲/清書・実サイトへの投稿・各種アカウント作成・外部サービスの操作
- **こちら（エンジニア）の担当**：コード実装・DB/マイグレーション・テナントビルダーの出力をコードへ反映・デプロイ・バグ修正・（依頼された場合の）較正ツール実行
- ⚠️ **コードの修正は必ずこのセッションで行う**。プランナーはコードレビューはするが**コードは触らない**（Claude.aiセッションなのでローカルファイルを編集できない）。プランナーからバグ報告や修正方針が来たら、**実装するのはこちら**。二重適用や衝突を避けるため、この分担を崩さない。
- ✅ **企画・方針への相互チェックは必ず行う（2アカウントの価値はここ）**。プランナーの方針に技術的な穴・矛盾・見落としがあれば、**遠慮せず・必要なだけ具体的に指摘する**（間違いを見逃す方が損失。軽く済ませることを優先しない）。一方で**雑談的な引き延ばしはしない**＝指摘は要点を押さえて。**質問文の代筆はしない**（本人の検閲・清書がポリシー違反回避の本体なので、ここは絶対に代行しない）。

### ⏭️ 次セッションでこちらが着手するタスク（ユーザーの指示待ち・順不同）
質問と回答が揃ってからの作業なので、**ユーザーから声がかかるまで着手しない**。
1. **Googleログインのみ化**：メールログインのUIを隠す（認証テーブル・データは削除しない。テストアカウントの切替に使っていたため、審査提出のタイミングで入口だけ塞ぐ方針）
2. **テスト/seed質問の削除**：現在の旧seedデータを削除するSQLを用意して実行（サイトが一時的に空になるので、本物の質問の準備ができた合図をもらってから）
3. **Guitar & Effector テナントの新規作成**：テナントビルダーの出力を5ファイルへ反映＋タグライン等を7言語へ翻訳＋閾値較正＋Cloudflareドメイン追加の案内（ビルダーの入力は一項目ずつ確認しながら進める）
4. その後の実環境への質問投入は、**プランナーが作るタイムスケジュールに従う**（こちらは貼るだけパケット等の技術支援のみ）
