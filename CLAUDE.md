...
**⚖️ ステマ規制（景表法の指定告示・2023年10月施行）との関係＝問題なし（2026-08-04 整理）**
- 規制対象は「**事業者の表示なのに第三者の表示であるかのように見せること**」。**自分の公式アカウントで自分のサービスの動画を上げるのは事業者の表示として明らかなのでステマではない**。「動画を上げたら流れるだけ」という理解で正しい。
- **ステマになるのは**: ①他人に依頼して宣伝させ、その依頼関係を隠す ②別人・一般ユーザーを装って勧める ③同一プラットフォーム内で複数アカウントを作って自演する。**どれもやらない**。
- **やるべき担保は1つだけ＝プロフィールに「自分が作った」と分かる記載を置く**（または動画側で明示）。これで疑義がゼロになる。既存方針の「質問者=運営者は隠さず出す」「作者明記」「誇張NG」と同じ。
- **1人が7媒体にアカウントを持つのは全く問題ない**（同一プラットフォーム内の複数アカウント自演とは別物）。
- **本当に表記が必要になるのはアフィリエイト**（PR表記必須）。ただし**AdSense承認後まで貼らない**方針なので現時点では無関係。
- **SNS→サイトの動線**: アカウントはテナント別に分けないので、プロフィールのリンクは**ルートポータル(wisdomassemble.com)に置いて各テナントへ分岐**させるのが基本。個別テナントへ送りたい動画は**キャプション/説明欄に直URL**を書く（YouTubeは説明欄自由・TikTokはプロフィール以外の外部リンクに制約あり）。**リンク先の言語は動画の言語と揃える**（英語版→/en、日本語版→/ja）。
