...
- ⚠️⚠️**【2026-08-25 実データで反例が出た】「ブレはAI公開率が下がる方向にしか効かない＝安全側」は誤りだった。** Day 9 で**2問が同時に逆方向へブレた**：
  ...
  - **G22の対応＝何もしない（放置して🔥へ）。** ⛔奥様は答えられない（本人の領域）／⛔本人が自分の質問に答えるのは自作自演でCLAUDE.mdの禁止事項／⭐「AIが答えられなかった質問が高難度に上がる」のは仕様が正しく動いている姿。回答ゼロが6問(9%)→7問(10.4%)に増えるが、AdSenseの危険水域(2割)には遠く許容範囲
...
| 理由 | 件数 | 評価 |
|---|---|---|
| noindex タグ | 659 | ✅ **設計どおり**（zh/id/vi/ko/es/pt の6言語＋bugテナント）。⚠️ただし手計算では約350にしかならず、残り約300は未確認 |
| クロール済み - インデックス未登録 | 150 | ⭐**ここだけが本物の課題**。Googleは来て読んだうえで「載せない」と判断している |
...
**⚠️ 未確認のまま残していること**
- noindex 659 の残り約300の正体 → GSCの「noindexタグによって除外されました」行をクリックしてサンプルURLを見れば分かる（30秒）
- 150件のクロール済み-未登録に**新67問が含まれているか** → 同じくサンプルURLで確認
...
- [x] ~~**C1'. サブドメインのsitemapをGSCに送信**~~ ✅**2026-08-27 完了。** ⚠️**それまでルートの1本しか登録されていなかった。** ただし**「知らせていなかったのが原因」は誤り**（2026-08-27訂正）。robots.txtが各ホストのsitemapを宣言済みなので発見経路はあった。実際の詰まりは **「クロール済み - インデックス未登録 150件」**＝Googleは来て読んだうえで載せないと判断している側で、これはsitemap送信では動かない。送信当日に読み込まれ guitar 46 / music-prod 44 ページを検出。⛔**GSCはドメインプロパティでもsitemapはホストごとに送る必要がある**
