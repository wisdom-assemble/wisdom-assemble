...
⛔⛔**【2026-08-25 保留】Facebookページのユーザーネームは、現在のUIで設定場所が見つからなかった。** プランナーが4ルート案内して全部行き止まりだったので、**同じ道をもう一度たどらないこと**。試して無かった場所は以下：

1. `ページの基本データ`タブ → カテゴリと連絡先だけ。ユーザーネーム欄なし
2. `設定とプライバシー → ページの詳細` → 戻されるだけ
3. `プロフィール右上の ✏️編集 → 基本データを編集` → 基本データタブに戻るループ
4. `Meta Business Suite → 設定 → アカウント → ページ` → **権限管理の画面**で、ページ情報の編集項目は無い

⚠️**最有力の理由：Facebookは作りたて（フォロワー0・投稿0）のページにユーザーネームをまだ開放していない。** 項目自体がどこにも無いことの説明がつく。**投稿が始まってから再挑戦する**のが正解で、いま粘っても出てこない。

**実害はない。** 他6媒体は `wisdomassemble` 系で揃っており、Facebookページへの導線はプロフィールリンクではなくReels経由。数字URL（`profile.php?id=1321490887709667`）のままでも困らない。
...
| 2 | **Facebookページ** | ✅ 2026-08-24 完了。カテゴリ「ウェブサイト」・リンクのみ設定・連絡先は全部空欄 |
...
- [ ] **A6. 投稿1問目で通しの確認**（**2026-08-08にDB側は検証済みだが、アプリ経由のフローは未通過**）
