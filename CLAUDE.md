...
**フロントエンド・デザイン**
7. 細かいUI/UXテストと修正（各画面ごと）
   - トップ：ローディングスケルトン・空状態UI
   - 質問詳細：長文折り返し・コードブロック対応（マークダウン表示）
   - 質問投稿：文字数カウンター・バリデーション表示
   - 回答フォーム：最小文字数バリデーション（20文字以上）
   - マイページ：スマホ時のタブ折り返し
   - 404・エラーページのデザイン
8. クローン化ルール整備（テナントごとにカラー・名称・ロゴ・コピー文を差し替え。現状はCSS変数 `--color-primary` のみ。フォント・ロゴ・favicon・ヘッダー文も設定可能にする）
9. 多言語対応（i18n。日本語ベースで英語対応。next-intl等の導入検討）
10. モバイル対応（ヘッダーナビのハンバーガーメニュー化・ボタン類のスマホ幅対応）
11. 質問本文のマークダウン・コードハイライト対応（技術系質問が多いため優先度高め）
12. OGP画像対応（SNSシェア時のサムネイル）

**法的ページ（リリース前必須）**
13. 利用規約ページ（/terms）
14. プライバシーポリシーページ（/privacy）- 収集情報：メールアドレス・Googleプロフィール名のみ
15. 特定商取引法に基づく表記（有料機能導入時）
16. フッターに利用規約・プライバシーポリシーへのリンク追加
17. Googleログイン時に「登録することで利用規約に同意したとみなします」文言追加

**実装済み（フェーズ②末）**
- ✅ 投稿直後に類似の解決済み質問を最大4件表示（`?result=` パラメータがある時のみ。タイトルキーワードでOR検索・status=solvedのみ対象）
