...
**⏭️ 承認後に戻す作業**: 🔥6問（M05 M29 M32／G14 G16 G17）に**書いてある回答を投稿する**。申請時は回答ゼロを9%に抑えるため意図的に伏せているだけで、回答自体は既にNotionにある。
...
### 🚩 次セッションの入口（2026-08-15 更新・ここから読む）

> **⚡エンジニアの残タスクはゼロ。次は投稿フェーズ（mtさん・プランナー）。**
>
> **このセッションで最初にやること**：mtさんから「1問投稿した」と連絡が来たら、
> `node scripts/verify-latest-post.mjs guitar 1` を実行して結果を報告する。それだけ。
> 連絡が来るまでは待機でよい（勝手にコードを触らない）。
>
> **2026-08-15に完了**：GUITAR & PEDALSテナント作成（A3）／Googleログインのみ化（B1）／About運営者情報（B5）／
> 多言語の穴5件・ロゴのTM2件・プランナー指摘4件の修正／**公開リポジトリに出ていたservice_roleキーの無効化**。
>
> **⚠️投稿は1問目で必ず止めること。** `node scripts/verify-latest-post.mjs guitar 1` で
> 翻訳8言語・AI回答・マッチングの割当・通知メールの4点を確認する。
> 8/08の回答トリガー修正と8/15のマッチング変更のあと、この経路はまだ通っていない
> （開発中は何度も投稿しているが、それらの変更より前）。2問目以降は止めなくてよい。
>
