...
**⏳ 未確認で残っていること**
- 150件のクロール済み-未登録に**新67問が含まれているか**
  → `ページのインデックス登録` → 下の理由テーブル → 「クロール済み - インデックス未登録」の行をクリック
  → サンプルに `/questions/` の新問（HX Stomp・Klon Centaur 等）があるか
  - `seed-*`／テスト投稿ばかり → ✅ 問題なし
  - 新問が混ざっている → ⚠️ Googleが新問を見て却下している＝要対応
