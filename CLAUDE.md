...
- **将来スケール（〜100テナント構想）**：承認後は"宣伝1発→放置"でスロー展開。開設ライン10問は上の分業手順で満たす（人間回答は後から実ユーザーが埋める）。**1〜3問しかないテナントはnoindexのまま**（薄いページはドメイン全体のSEO/審査を下げる）。展開ペースは**承認まで&直後は月1〜2本**、その後は品質維持を条件に**月3〜4本（週1・年50本）まで許容**。一気の量産はscaled-content判定リスク。判断軸はカレンダーでなく質と各テナントの生存シグナル。Search Consoleで手動対策/インデックス異常を監視（カナリア）。
