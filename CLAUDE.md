...
- [ ] **B6. 質問詳細のソフト404を直す**（削除済み質問URLが404でなく200を返す。原因は`loading.tsx`のストリーミング。実験で確認済み。要トレードオフ判断）— エンジニア
...
4. **質問詳細のソフト404**（下記2026-08-08参照）の対応判断
...
#### ⑤ 未解決: 質問詳細ページのソフト404（**要判断・未対応**）
```
/ja/no-such-page              → 404 ✅
/ja/questions/<削除済みslug>  → 200 ❌（中身は「見つかりません」ページ）
```
- 原因: 質問詳細ルートの **`loading.tsx` によるストリーミング**で、`notFound()` が走る前に 200 のヘッダーが送信済みになる
- **本番相当環境（`next start`）で `loading.tsx` を外して再ビルドすると 404 に変わることを実験で確認済み**
- 影響: **MUSIC PRODUCTION の削除済み約25件のURLが「200を返すが中身の無いページ」**として残る。GSCに「ソフト404」として出て、*Low value content* と同じシグナルになる。BUG側は全ページnoindexなので放置でも消える
- トレードオフ: `loading.tsx` を外すと質問詳細のスケルトン表示が無くなる。**折衷案＝`loading.tsx` を外し、回答一覧だけページ内 `Suspense` で包む**
