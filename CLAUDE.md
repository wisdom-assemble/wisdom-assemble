...
#### ⚠️バナーの「自動生成」について（2026-08-25 プランナーが誇張 → mtさん指摘で訂正）

⛔⛔**プランナーは画像を作れない。デザインできないし、描けない。** できるのは「**文字・色・図形をコードで組んでPNGに書き出す**」ことだけ。既存のOG画像がまさにそれで、中身は「色の上にワードマークを置いて影を付ける」レイアウト処理。

⚠️**さらに「既に動いている仕組みがある」と書いたが、動いていない**（実ファイルで再確認した）：

```
src/app/opengraph-image.tsx  → 存在しない
public/og/*.png              → ただの静止画（2026-07-30 / 08-15 に作られたPNG 4枚）
layout.tsx:101               → その静止画を直接参照しているだけ
```

`scripts/image-templates/opengraph-image.tsx` は**アプリから外して置いてあるテンプレ**であって、稼働中のコードではない。
...
⚠️**教訓：ファイルが存在することと、それが動いていることは別。** `scripts/` にあるコードを見て「仕組みが動いている」と書いたのが誤り。**アプリに配線されているかまで確認すること。**
