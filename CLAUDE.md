...
- **バナー**：型さえあれば複製は機械的なので**テナント数に比例しにくい**。⚠️ただし手順は自動ではなく、**2311行の5手順（テンプレを `src/app/` へ戻す→デプロイ→書き出し→退避→再デプロイ）を毎回踏む**必要がある。⚠️**投稿の頻度は別問題**で、リリース告知を立て続けに撒くと広告アカウントに見える。**素材が作れることと、出していいことは別**
