...
**【A. コンテンツ】**
- [ ] **A1. 旧シード削除**（偽ペルソナ50問＋重複32件・SQLのみ・Workerに影響なし）— エンジニア
- [ ] **A2. BUG休眠化**（noindex＋sitemap除外＋`PortalHome.tsx`の`REVIEW_TENANT_IDS`から`bug`削除）— エンジニア
...
- [ ] **B2. 新テナントの画像を用意**：`public/icons/guitar.png`(32x32)と`public/og/guitar.png`(1200x630)。**無いとファビコン/OGPが404になる**（2026-07-30のA対応で静的ファイル化したため）
