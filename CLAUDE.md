...
| ③ **回答待ち6問** | ⚠️うち**2問は8/24に投稿したばかり（経過3時間）＝正常**。実際に溜まっているのは**古い4問**で、最長は8/21のギター「70年代のストラトのマルチ設定について」の**75時間**。計画では「回答ゼロは6問(9%)まで」なので、**放置すれば枠を食う** |
...
#### ⚠️「期限切れなのに `open` のまま」はバグではない（実コードで確認）

`escalate/route.ts` を読んで確認した。**期限切れで勝手に🔥へ飛ぶcronは存在しない。** 🔥になるのは次の2つだけ。

1. **投稿時にAIスコアが閾値(91)に届かなかったとき**（`api/questions/route.ts` が `status:'hard'` を直接書く）← 8/23のM05「USBケーブルの質で音質は変わる？」がこれ
2. **質問者か回答者が明示的に操作したとき**（`escalate` API）。**matched_b_deadline が切れても、UIに「高難度へ」ボタンが出るようになるだけ**

つまり `open` の6問は**仕様どおり人間の回答を待っている**状態。⛔ただし**放置すると回答ゼロのまま残り、9%の枠を食う**ので、遅れは遅れとして解消すること。
...
#### ⭐️バナーは自動生成できる（既存の仕組みが使える・プランナー発見）

⚠️⚠️**`scripts/image-templates/opengraph-image.tsx` が既にテナント別の画像ジェネレータとして動いている。** `next/og`（Satori）で、**テナントIDを渡すと色と名前を差し替えて1200×630のPNGを吐く**。実際に `public/og/` に dtm・guitar・debug・root の4枚が生成済み。

```tsx
const tenantId = (await headers()).get('x-tenant-id') ?? 'debug'
const label = TENANT_NAME_MAP[tenantId] ?? 'WISDOM ASSEMBLE'
const primary = OG_COLORS[tenantId] ?? DEFAULT_COLOR
```

**これを出力サイズだけ変えて回せば、リリースバナーの4サイズ（1080正方形 / 4:5 / 16:9 / 2:3 Pinterest）がテナントごとに自動で出る。**

- ⭐️**100テナントでもデザイン作業はゼロ。** `tenants` テーブルに `color_theme` と `name` があるので、テナントを作った瞬間にバナーも揃う
- ⛔**ただし手作りの1枚には勝てない。** 「ブランドの型」を1つ決める初期投資（mtさんの領分）は必要で、**そこから先だけが自動化できる**という話
- ⚠️**やるなら「OG画像の英文差し替え」（AdSense承認後タスク）と同時にやること。** 古いコピー `AI answers first — humans fill the gaps` が焼き込まれたままなので、どうせ再生成が要る。**別々にやると二度手間**
- ⚠️**自動化できるのはバナーだけ。動画は自動生成できない**（映像・音声処理は別ツール）。動画は従来どおり「1テナント＝1本」を人が作る
...
- **バナー**：自動生成なら**テナント数に比例しない**。⚠️ただし**投稿の頻度は別問題**で、リリース告知を立て続けに撒くと広告アカウントに見える。**素材が作れることと、出していいことは別**
...
| 6 | **TikTok** | ⚠️**未着手。8/24の予定を飛ばしていた**（2026-08-25にmtさんの指摘で発覚）。動画7媒体の筆頭なので、これが無いと横展開が成立しない |
...
新アカウント2件がログインした結果、**奥様と管理アカウントも回答候補になっていた**（このままだと mtさんの質問が奥様に割り当たり、通知メールが飛んで8時間後に自動で高難度化する）。SQLで `is_available=false` にして是正済み。現在の回答候補は **columbusslx1981（YOYOYO）の dtm / guitar の2行のみ**。

```sql
update tenant_profiles tp set is_available = false
from auth.users u
where tp.user_id = u.id
  and lower(u.email) in ('deadman199478@gmail.com', 'wisdomassemble@gmail.com');
```

#### 🚧 実ユーザーを迎える前に片付ける項目（この2件はセットで扱う）

**(a) 新アカウントは既定で回答候補になる**
`tenant_profiles.is_available` の既定は `true` で、8/8に追加した `ensureTenantMembership`（ログインするとその テナントの住人として行を作る）で新規作成される。**一般ユーザーは回答者候補であってほしい**ので仕組みとしては正しいが、seed期は「奥様＝質問専用」という特殊運用なので都度外す必要がある。**新しいテナントを作って奥様がそこにログインすると再発する**（上のSQLを流す）。実ユーザーが来る段階では、稼働トグルの復活か、既定を false にするかの判断が要る。
