...
function MatchingOverlay({ label }: { label: string }) {
  return (
...
export function AcceptButton({ questionId, answerId }: { questionId: string; answerId: string }) {
  ...
  return (
    ...
    >
      {loading ? t('acceptConfirmProcessing') : t('acceptButton')}
    </button>
  )
}
...
export function RematchButton({ questionId }: { questionId: string }) {
  ...
  async function rematch() {
    if (!confirm(t('rematchConfirm'))) return
    setLoading(true)
    setShowOverlay(true)
  ...
  if (done) return <p className="text-xs text-gray-400 mt-2 text-center">{t('rematchDone')}</p>

  return (
    <>
      {showOverlay && <MatchingOverlay label={t('matching')} />}
      <button
        onClick={rematch}
        disabled={loading}
      ...
      >
        {loading ? t('acceptConfirmProcessing') : t('rematchButton')}
      </button>
    </>
  )
}
...
export function EscalateHardButton({ questionId }: { questionId: string }) {
  ...
  async function escalate() {
    if (!confirm(t('escalateConfirm'))) return
    setLoading(true)
    await fetch(`/api/questions/${questionId}/escalate`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  if (done) return <p className="text-xs text-red-500 mt-2 text-center">{t('escalateDone')}</p>
  ...
  return (
    ...
    >
      {loading ? t('acceptConfirmProcessing') : t('escalateButton')}
    </button>
  )
}
...
export function GiveUpButton({ questionId }: { questionId: string }) {
  ...
  if (done) return <p className="text-xs text-gray-400 mt-2">{t('giveUpDone')}</p>
  ...
  return (
    ...
    >
      {loading ? t('acceptConfirmProcessing') : t('giveUpButton')}
    </button>
  )
}
