...
const GENRE_CONFIG: Record<string, {
  ...
  guitar: {
    label: 'ギター・ペダル（エフェクター）',
    threshold: 91,
    // 日本語と英語の両方の言い方（エフェクター / ペダル / pedals）を入れてある。
    // ここはAIがジャンル内外を判定するために読む説明文なので、どちらの言語で
    // 質問が来ても正しく判定させたい＝一方に決める必要がない。
    inScope: 'エレキギター・アコースティックギター・アコギ・ペダル・エフェクター・pedals・マルチエフェクター・アンプ・真空管アンプ・アンプシミュレーター・ピックアップ・配線・改造・リペア・ビンテージ機材・セットアップ・調整・弦・レコーディング・ペダルボード',
    outScope: '料理・スポーツ・恋愛・プログラミング・税金など、ギターや機材と無関係な話題',
    // 真空管アンプは電源を切ってもコンデンサに致命的な電圧が残る。
    // AIが自信満々に間違えると人が死ぬ領域なので、スコアを下げて必ず人間へ回す。
    dangerKeywords: /感電|高電圧|発火|やけど/,
  },
}
