...
-- 2. questions に tags（質問タグ）を追加（将来のタグフィルター検索用）
ALTER TABLE questions ADD COLUMN IF NOT EXISTS tags text[] NOT NULL DEFAULT '{}';

-- 3. check_and_award_titles関数を更新（active_title_idを自動で最高称号に設定）
...
-- 5. テストユーザーのcheck_and_award_titlesを全員実行（既存データに称号付与）
DO $$
DECLARE
  r record;
BEGIN
  FOR r IN SELECT id FROM profiles LOOP
    PERFORM check_and_award_titles(r.id);
  END LOOP;
END;
$$;
