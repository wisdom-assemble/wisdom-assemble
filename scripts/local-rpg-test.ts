...
const PERSONAS: Persona[] = [
  ...
  {
    id: 'john', name: 'John', email: 'john@test.com', class: '野心家ハッカー（途中参加）',
    ...
    isResting: false, joinAfterQ: 20,
  },
  {
    id: 'anthony', name: 'Anthony', email: 'anthony@test.com', class: 'ソーシャルエンジニア（途中参加）',
...
const QUESTIONS: Q[] = [
  ...
  { id:2,  category:'React',      difficulty:'easy',   expected:'correct',      title:'useEffectが無限ループする', body:'useEffect内でsetStateを呼んでいます。依存配列に[]を渡しているのに無限ループになります。原因はなんですか？' },
  { id:3,  category:'React',      difficulty:'easy',   expected:'correct',      title:'Propsをコンポーネントに渡せない', body:'親コンポーネントから子コンポーネントにonClick関数をPropsで渡しています。子側でprops.onClickを呼ぶと「is not a function」エラーが出ます。' },
  ...
  { id:5,  category:'React',      difficulty:'medium', expected:'partial',      title:'useContextで値が更新されない', body:'React ContextのProviderでvalueを更新しても、Consumerのコンポーネントが再レンダリングされません。ContextはuseStateで管理しています。' },
  ...
  { id:7,  category:'Next.js',    difficulty:'easy',   expected:'correct',      title:'Next.jsでimport画像がエラーになる', body:'Next.jsでimport logo from "../../public/logo.png"としたら「cannot find module」エラーが出ます。next.config.jsの設定が必要ですか？' },
  ...
  { id:9,  category:'Next.js',    difficulty:'medium', expected:'partial',      title:'Next.jsのMiddlewareが本番でだけ動かない', body:'middleware.tsをプロジェクトルートに置いています。ローカルでは動くのに、Vercelにデプロイすると無視されます。matcherの設定は合っています。' },
  ...
  { id:13, category:'Next.js',    difficulty:'easy',   expected:'correct',      title:'next/imageでExternal URLの画像が表示されない', body:'next/imageコンポーネントで外部URLの画像を表示しようとすると「hostname is not configured」エラーが出ます。' },
  { id:14, category:'React',      difficulty:'medium', expected:'partial',      title:'Zustandのstoreが複数タブで同期しない', body:'ZustandでグローバルなCartステートを管理しています。タブAで追加した商品がタブBに反映されません。どうすれば同期できますか？' },
  ...
  { id:17, category:'TypeScript', difficulty:'easy',   expected:'correct',      title:'TypeScriptでObject is possibly undefinedエラー', body:'TypeScriptで「Object is possibly undefined」エラーが大量に出ます。Optional Chainingの使い方を教えてください。' },
  ...
  { id:19, category:'TypeScript', difficulty:'easy',   expected:'correct',      title:'TypeScriptでas constとreadonlyの違い', body:'TypeScriptでas constとreadonlyはどう違いますか？どちらを使うべきか判断基準を教えてください。' },
  { id:20, category:'TypeScript', difficulty:'medium', expected:'partial',      title:'TypeScript 5.xのdecoratorsが動かない', body:'TypeScript 5.xでデコレーターを使おうとしています。「Decorators are not valid here」エラーが出ます。tsconfig.jsonの設定が必要ですか？' },
  ...
  { id:22, category:'TypeScript', difficulty:'easy',   expected:'correct',      title:'TypeScriptでenumの値を文字列として使いたい', body:'TypeScriptのenumをAPIレスポンスの文字列と比較したいです。enumの値を文字列として取得する方法を教えてください。' },
  { id:23, category:'TypeScript', difficulty:'medium', expected:'partial',      title:'TypeScriptのpath aliasがJestだけで解決できない', body:'tsconfig.jsonでpathsを設定してimport aliasを使っています。Jestでテストを実行するとModule not foundエラーになります。' },
  ...
  { id:25, category:'SQL',        difficulty:'easy',   expected:'correct',      title:'PostgreSQLでNULLの比較がおかしい', body:'PostgreSQLでWHERE column = NULLとしてもレコードが取得できません。NULLの比較方法を教えてください。' },
  ...
  { id:27, category:'SQL',        difficulty:'medium', expected:'partial',      title:'N+1問題をSQLで解消したい', body:'ORMを使っているとN+1問題が発生します。JOINを使って1クエリで解消する方法と、ORMでの対処法を教えてください。' },
  ...
  { id:29, category:'SQL',        difficulty:'hard',   expected:'needs_human',  title:'本番DBのSELECTが突然遅くなった', body:'昨日まで100ms以下だったクエリが、突然5秒以上かかるようになりました。データ量は変わっていません。インデックスは存在します。' },
  { id:30, category:'SQL',        difficulty:'easy',   expected:'correct',      title:'SQLでGROUP BYとHAVINGの使い方', body:'SQLでGROUP BYとHAVINGの違いがわかりません。集計関数（COUNT, SUM）と組み合わせた使い方を教えてください。' },
  { id:31, category:'SQL',        difficulty:'medium', expected:'partial',      title:'PostgreSQLのJSONBカラムへのインデックス', body:'PostgreSQLのJSONBカラムに対してインデックスを張りたいです。GINインデックスとBTreeインデックスの使い分けを教えてください。' },
  ...
  { id:33, category:'SQL',        difficulty:'medium', expected:'partial',      title:'PostgreSQLでUPSERTを使いたい', body:'PostgreSQLでINSERT OR UPDATE（UPSERT）を実装したいです。ON CONFLICT句の使い方を教えてください。' },
  ...
  { id:35, category:'Docker',     difficulty:'easy',   expected:'correct',      title:'Dockerコンテナ間で通信できない', body:'Docker Composeで2つのサービスを立ち上げています。serviceAからserviceBのポートに接続できません。ネットワーク設定はどうすればいいですか？' },
  ...
  { id:37, category:'Docker',     difficulty:'medium', expected:'partial',      title:'DockerコンテナのNode.jsがメモリ不足でクラッシュ', body:'DockerコンテナでNode.jsアプリを動かしていますが、メモリ制限に引っかかって落ちます。NODE_OPTIONS=--max-old-space-sizeの設定方法を教えてください。' },
  ...
  { id:39, category:'Docker',     difficulty:'easy',   expected:'correct',      title:'DockerfileのCMDとENTRYPOINTの違い', body:'DockerfileのCMDとENTRYPOINTの違いを教えてください。どちらをいつ使うべきですか？' },
  { id:40, category:'Docker',     difficulty:'medium', expected:'partial',      title:'Docker Composeのvolumesでファイルが同期しない', body:'Docker Composeでローカルのソースコードをコンテナにマウントしています。ローカルでファイルを更新してもコンテナに反映されません。' },
  ...
  { id:42, category:'Docker',     difficulty:'medium', expected:'partial',      title:'マルチステージビルドで最終イメージが大きい', body:'Dockerfileでマルチステージビルドを使っているのに、最終イメージが1GB以上になります。Node.jsアプリです。サイズを小さくするコツを教えてください。' },
  { id:43, category:'Docker',     difficulty:'easy',   expected:'correct',      title:'Dockerで環境変数を.envファイルから読み込みたい', body:'Docker Composeで.envファイルの環境変数をコンテナに渡す方法を教えてください。env_fileとenvironmentの使い分けも知りたいです。' },
  ...
  { id:45, category:'AWS',        difficulty:'medium', expected:'partial',      title:'Lambda関数のコールドスタートを改善したい', body:'AWS LambdaのNode.js関数のコールドスタート時間が3秒以上あります。改善方法を教えてください。Provisioned Concurrencyは使いたくないです。' },
  { id:46, category:'AWS',        difficulty:'medium', expected:'partial',      title:'API GatewayとLambdaでCORSエラーが出る', body:'API GatewayとLambdaで構築したAPIをブラウザから叩くとCORSエラーが出ます。API GatewayのCORS設定とLambdaのレスポンスヘッダーの両方に設定が必要ですか？' },
  ...
  { id:49, category:'AWS',        difficulty:'medium', expected:'partial',      title:'CloudFrontのキャッシュが即座に反映されない', body:'S3に新しいファイルをアップロードしたのに、CloudFront経由でアクセスすると古いファイルが返ってきます。キャッシュを即座にクリアする方法を教えてください。' },
  { id:50, category:'AWS',        difficulty:'easy',   expected:'correct',      title:'IAMロールとIAMユーザーの違い', body:'AWSのIAMロールとIAMユーザーの違いを教えてください。EC2やLambdaにはどちらを使うべきですか？' },
  { id:51, category:'AWS',        difficulty:'hard',   expected:'needs_human',  title:'ECSタスクが断続的にExitCode137で終了する', body:'AWS ECSでコンテナが断続的にExitCode137で終了します。OOM Killerだと思いますが、タスク定義のメモリ制限は十分あるはずです。' },
  { id:52, category:'AWS',        difficulty:'hard',   expected:'needs_human',  title:'SQSのメッセージが重複処理される', body:'AWS SQSでメッセージを処理するLambdaがあります。1つのメッセージが2回処理されることがあります。冪等性をどう担保すればいいですか？' },
  ...
  { id:55, category:'Supabase',   difficulty:'medium', expected:'partial',      title:'SupabaseのRLSでINSERTだけ403になる', body:'SupabaseのSELECTは成功するのに、INSERTだけ403エラーになります。RLSポリシーはSELECTとINSERT両方設定しました。' },
  { id:56, category:'Supabase',   difficulty:'easy',   expected:'correct',      title:'Supabaseのリアルタイム機能の使い方', body:'SupabaseのリアルタイムサブスクリプションをReactで使いたいです。テーブルの変更をリアルタイムに受け取る方法を教えてください。' },
  ...
  { id:58, category:'Supabase',   difficulty:'medium', expected:'partial',      title:'SupabaseのJWTトークンが期限切れで401になる', body:'SupabaseクライアントでAPIを叩くと定期的に401エラーが出ます。JWTの有効期限が切れているようです。自動リフレッシュの設定方法を教えてください。' },
  ...
  { id:60, category:'Supabase',   difficulty:'hard',   expected:'needs_human',  title:'SupabaseのRLSパフォーマンスが著しく低下', body:'RLSを有効にした後からクエリの速度が10倍遅くなりました。100万件のテーブルです。インデックスは設定済みです。' },
  ...
  { id:62, category:'Supabase',   difficulty:'medium', expected:'partial',      title:'SupabaseでGoogleログインを実装したい', body:'SupabaseでGoogle OAuthを使ったソーシャルログインを実装したいです。設定手順とNext.jsでの実装方法を教えてください。' },
  ...
  { id:64, category:'CSS',        difficulty:'easy',   expected:'correct',      title:'CSSのz-indexが効かない', body:'CSSでz-indexを設定しても要素が期待通りに重ならない問題があります。z-indexが効くための条件を教えてください。' },
  ...
  { id:66, category:'CSS',        difficulty:'easy',   expected:'correct',      title:'CSSのGrid Layoutで均等な列を作りたい', body:'CSS Gridで均等な3列レイアウトを作りたいです。repeat()とfr単位の使い方を教えてください。' },
  { id:67, category:'CSS',        difficulty:'medium', expected:'partial',      title:'レスポンシブデザインでモバイルだけ崩れる', body:'デスクトップでは正常に見えるレイアウトが、スマートフォンで崩れます。特定の端末だけで発生します。デバッグ方法を教えてください。' },
  { id:68, category:'CSS',        difficulty:'medium', expected:'partial',      title:'CSSアニメーションがiOSで動かない', body:'CSS transitionとkeyframesアニメーションがiOS Safariでだけ動きません。他のブラウザでは正常です。' },
  { id:69, category:'CSS',        difficulty:'medium', expected:'partial',      title:'Next.jsでCSSモジュールとTailwindが競合する', body:'Next.jsでCSS ModulesとTailwind CSSを混在させています。クラス名の優先度が意図しない結果になります。' },
  ...
  { id:71, category:'CSS',        difficulty:'medium', expected:'partial',      title:'CSSのカスタムプロパティの使い方', body:'CSSのカスタムプロパティ（--my-color: #ff0000など）の使い方を教えてください。JavaScriptから変更することもできますか？' },
  ...
  { id:73, category:'Python',     difficulty:'easy',   expected:'correct',      title:'Pythonで非同期処理をしたい', body:'Pythonでasync/awaitを使った非同期処理の基本的な書き方を教えてください。asyncioとは何ですか？' },
  { id:74, category:'Python',     difficulty:'medium', expected:'partial',      title:'PythonのVirtual Environmentが毎回リセットされる', body:'Python venvで環境を作って pip installしましたが、ターミナルを再起動するとパッケージが見つかりません。' },
  { id:75, category:'Python',     difficulty:'easy',   expected:'correct',      title:'Pythonで日付の計算をしたい', body:'Pythonで日付の差分を計算したいです。datetimeモジュールを使って「今日から30日後」などを求める方法を教えてください。' },
  ...
  { id:78, category:'Python',     difficulty:'medium', expected:'partial',      title:'PythonのPandasでメモリ不足になる', body:'Pandasで1GBのCSVファイルを読み込もうとするとメモリエラーになります。大きなファイルを効率的に処理する方法を教えてください。' },
  ...
  { id:81, category:'セキュリティ', difficulty:'medium', expected:'partial',    title:'CSRF対策をSPA+APIで実装したい', body:'ReactのSPAとAPIサーバーが別ドメインにあります。CSRF攻撃への対策はどうすればいいですか？Cookieを使った認証です。' },
  ...
  { id:84, category:'セキュリティ', difficulty:'hard',   expected:'needs_human', title:'本番サーバーでJWTが突然全員無効になった', body:'昨日のデプロイ後から全ユーザーのJWTが無効になっています。JWT_SECRETの環境変数は変更した覚えはありません。デプロイはDockerイメージの更新だけです。' },
  { id:85, category:'セキュリティ', difficulty:'medium', expected:'partial',    title:'パスワードのハッシュ化にMD5を使っている', body:'既存のシステムがパスワードをMD5でハッシュ化して保存しています。bcryptに移行したいのですが、既存ユーザーのパスワードはどう扱えばいいですか？' },
  { id:86, category:'セキュリティ', difficulty:'hard',   expected:'needs_human', title:'XSS攻撃の痕跡をログで見つけた', body:'本番のアクセスログで、クエリパラメータに<script>タグが含まれるリクエストを大量に発見しました。現在のところ被害は確認できていません。対応策を教えてください。' },
  { id:87, category:'セキュリティ', difficulty:'medium', expected:'partial',    title:'OAuthのcallback URLが奪取される可能性', body:'GoogleのOAuth 2.0でログイン機能を実装しています。redirect_uriをユーザーが操作できる状態になっていることを指摘されました。どう対処すればいいですか？' },
  ...
  { id:90, category:'Git',         difficulty:'easy',   expected:'correct',     title:'Gitのマージコンフリクトの解消方法', body:'git mergeでコンフリクトが発生しました。コンフリクトマーカーの意味と解消手順を教えてください。' },
  ...
  { id:92, category:'Git',         difficulty:'medium', expected:'partial',     title:'git rebaseでコミット履歴を整理したい', body:'featureブランチに10個のコミットがあります。mainにマージする前に、関連するコミットをまとめて1つにしたいです。interactive rebaseの使い方を教えてください。' },
  ...
  { id:94, category:'Git',         difficulty:'medium', expected:'partial',     title:'GitHub Actionsでnpm installが毎回フルインストールされる', body:'GitHub ActionsでNode.jsのCIを実行しています。キャッシュアクションを設定しましたがnpm installが毎回フルで走ります。' },
...
async function insertQuestion(questioner: Persona, q: Q, aiScore: number, aiAnswer: string | null): Promise<string | null> {
  const ts = Date.now()
  const slug = `t5-q${q.id}-${ts}`
  const status = aiAnswer ? 'ai_answered' : 'open'
  const { data, error } = await db.from('questions').insert({
    tenant_id: 'debug',
    user_id: questioner.uid!,
    title: q.title,
    body: q.body,
    slug,
    status,
  ...
  if (aiAnswer) {
    await db.from('answers').insert({
      question_id: data.id, tenant_id: 'debug',
      body: aiAnswer, is_ai: true, ai_score: aiScore,
    })
  }
  return data.id
}

async function insertAnswer(questionId: string, responder: Persona, body: string) {
  await db.from('answers').insert({
    question_id: questionId, tenant_id: 'debug',
    user_id: responder.uid!, body, is_ai: false,
  })
}

async function resolveQuestion(questionId: string, resolverId: string, answerId: string | null) {
  await db.from('questions').update({
    status: 'solved',
    solved_at: new Date().toISOString(),
    solved_by: resolverId,
  }).eq('id', questionId)
  if (answerId) {
    await db.from('answers').update({ is_accepted: true }).eq('id', answerId)
    await db.rpc('increment_answer_count', { uid: resolverId }).then(() => {}).catch(() => {})
  }
}
...
interface QResult {
  ...
  questioner: string; aiScore: number
  resolution: 'ai_solved' | 'B_solved' | 'C_solved' | 'high_quest'
  responderB: string | null; responderC: string | null
  timeLimit: TimeLimit; narrative: string
}
...
async function runSimulation() {
  ...
  for (let i = 0; i < QUESTIONS.length; i++) {
    const available = personas.filter(p => p.joinAfterQ <= i && p.uid)
  ...
  for (let qi = 0; qi < QUESTIONS.length; qi++) {
    ...
    const label = `[${String(q.id).padStart(3)}] ${q.title.substring(0, 32).padEnd(32)} [${questioner.name.padEnd(8)}]`
    ...
    if (ai.score >= AI_THRESHOLD) {
      ...
      questionId = await insertQuestion(questioner, q, ai.score, ai.answer)
    } else {
      process.stdout.write(`AI❌(${ai.score}) `)
      narrative = `${questioner.name}が質問。AIスコア${ai.score}で閾値未満。人間へルーティング。`
      questionId = await insertQuestion(questioner, q, ai.score, null)
      ...
      if (!personaB) {
        ...
        if (questionId) await escalateToHard(questionId)
      } else {
        ...
        if (!bTimeout && !bGiveup) {
          ...
          narrative += ` ${personaB.name}（${personaB.class}）がマッチング。スキルスコア${matchB.toFixed(0)}で解決。`
          if (questionId) {
            await insertAnswer(questionId, personaB, `${q.title}について回答します。${q.body.slice(0, 30)}…の問題は、設定と環境を見直すことで解決できます。`)
            const { data: ans } = await db.from('answers').select('id').eq('question_id', questionId).eq('is_ai', false).single()
            if (ans) await resolveQuestion(questionId, personaB.uid!, ans.id)
          }
        } else {
          ...
          if (!personaC) {
            ...
            if (!cTimeout && !cGiveup) {
              ...
              if (questionId) {
                await insertAnswer(questionId, personaC, `追加で確認したところ解決策がわかりました。${q.title}はこの方法で対処できます。`)
                const { data: ans } = await db.from('answers').select('id').eq('question_id', questionId).eq('is_ai', false).maybeSingle()
                if (ans) await resolveQuestion(questionId, personaC.uid!, ans.id)
                else await db.from('questions').update({ status: 'solved', solved_at: new Date().toISOString(), solved_by: personaC.uid! }).eq('id', questionId)
              }
            } else {
    ...
    console.log()
    results.push({ id: q.id, title: q.title, category: q.category, difficulty: q.difficulty, questioner: questioner.name, aiScore: ai.score, resolution, responderB, responderC, timeLimit, narrative })
    await sleep(200)
  }
  ...
  console.log('📊 集計結果')
  console.log('='.repeat(60))
  console.log(`AI解決:         ${aiSolved}問 (${(aiSolved/total*100).toFixed(1)}%)`)
  console.log(`人間B解決:      ${bSolved}問 (${(bSolved/total*100).toFixed(1)}%)`)
  console.log(`人間C解決:      ${cSolved}問 (${(cSolved/total*100).toFixed(1)}%)`)
  console.log(`高難度クエスト: ${highQuest}問 (${(highQuest/total*100).toFixed(1)}%)`)
  console.log()
  console.log('ペルソナ別実績:')
  for (const p of personas) {
  ...
  for (const cat of cats) {
    ...
    console.log(`  ${cat.padEnd(15)}: ${hq}/${catR.length} (${(hq/catR.length*100).toFixed(0)}%)`)
  }
  ...
  if (!NOTION_TOKEN) { console.log('NOTION_TOKEN未設定 → スキップ'); return }
  console.log('\n📝 Notionに書き込み中...')

  const summaryBlocks: any[] = [
    nHeading(`🎮 テスト⑤ 実行結果  ${runDate}`),
    nText(`AI閾値: ${AI_THRESHOLD}  ペルソナ: ${personas.length}人  問題: ${QUESTIONS.length}問  実DB書き込み済み`),
    nHeading('📊 解決経路サマリー', 3),
    nCode([
      `AI解決:         ${aiSolved}問 (${(aiSolved/total*100).toFixed(1)}%)`,
      `人間B解決:      ${bSolved}問 (${(bSolved/total*100).toFixed(1)}%)`,
      `人間C解決:      ${cSolved}問 (${(cSolved/total*100).toFixed(1)}%)`,
      `高難度クエスト: ${highQuest}問 (${(highQuest/total*100).toFixed(1)}%)`,
    ].join('\n')),
    nHeading('👥 ペルソナ別実績', 3),
    nCode(personas
  ...
  for (let i = 0; i < results.length; i += 20) {
    ...
    const blocks: any[] = chunk.flatMap(r => {
      const icon = r.resolution === 'ai_solved' ? '🤖' : r.resolution === 'B_solved' ? '🟢' : r.resolution === 'C_solved' ? '🟡' : '🔴'
      return [
        nText(`${icon} Q${r.id} [${r.category}/${r.difficulty}] ${r.title}`),
        nText(r.narrative),
      ]
    })
